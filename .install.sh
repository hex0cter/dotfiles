...
install_macos() {
  ...
  brew update
  brew install coreutils git tmux zsh go asdf fzf autojump bat wget curl fd gpg git-crypt git-lfs git-secret libmagic awscli docker pipenv virtualenv libjpeg the_silver_searcher
